...
class IslandRenderer:
    ...
    def __init__(self):
        self.islandShader = Shader("res/shaders/IslandShader.txt")

    def render(self, islands, projMatrix, camera, lights):
        glEnable(GL_DEPTH_TEST)
        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        self.islandShader.bind()
        self.islandShader.setUniformMat4fv("projMatrix", projMatrix)
        self.islandShader.setUniformMat4fv("viewMatrix", camera.getViewMatrix())
        self.islandShader.setUniform4f("colour", 0, 1, 0, 1)
        for i in range(self.MAX_LIGHTS):
            if i < len(lights):
                light = lights[i]
                self.islandShader.setUniform3f(f"lightPosition[{i}]", *light.position)
                self.islandShader.setUniform3f(f"lightColour[{i}]", *light.colour)
            else:
                self.islandShader.setUniform3f(f"lightPosition[{i}]", 0, 0, 0)
                self.islandShader.setUniform3f(f"lightColour[{i}]", 0, 0, 0)
        for island in islands:
            self.islandShader.setUniformMat4fv("modelMatrix",
                                               pyrr.matrix44.create_from_translation(pyrr.Vector3([0, 0, 0])))
            glBindVertexArray(island.model.getID())
            glEnableVertexAttribArray(0)
            glEnableVertexAttribArray(1)
            glDrawArrays(GL_TRIANGLES, 0, island.model.getVertexCount())
        glDisableVertexAttribArray(0)
        glDisableVertexAttribArray(1)
        glBindVertexArray(0)
        glDisable(GL_DEPTH_TEST)
...
class MasterRenderer:
    ...
    def __init__(self):
        ...
        self.islandRenderer = IslandRenderer()
    ...
    def renderScene(self, camera, entities, islands, lights):
        ...
        self.islandRenderer.render(islands, self.projectionMatrix, camera, lights)
